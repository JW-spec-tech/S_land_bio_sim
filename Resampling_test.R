...
Resample_write_PBfall <- function(n=20,cores=as.numeric(Sys.getenv('OMP_NUM_THREADS'))) {
...
Resample_sims_random <- function(j,percent_f=0.025){
  
  n.cores <- cores   #### Set number of CPU cores ####
  ...
  Resampling_list_2 <- foreach(
    ...
    S_data <- F_data_prop %>% 
      ...
      purrr::map_dfr(~slice_sample(.x,n=.x$prop_random[1])) %>% 
      dplyr::bind_rows() %>% 
      dplyr::mutate(biomass=rTweedie((biomass*1000), p = 1.76, phi= 2))
    
    # store said sample
    Resampling_list[[i]] <- S_data
    Resampling_list[[i]]
  }
  ...
  foreach(
    q = 1:length(Resampling_list_2),
    .packages = c('mgcv','dplyr','purrr','arrow')
  ) %dopar% {
    cwd <- getwd()
    setwd("Resamples/")
    fname=paste0("PB_fall.dat",q)
    write.table(Resampling_list_2[[q]], file = fname,sep = " ", quote = F, row.names = F )
    setwd(cwd)
  }
...
# debugonce(Resample_write_PBfall) 254136
# Resample_write_PBfall(n=10)
